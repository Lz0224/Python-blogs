    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if errors == []:
                ...
                user = auth.authenticate(username = request.POST.get('name'), password = make_password(request.POST.get('password')))
        ...
        return render(request, 'failure.html', {'errors':e})
...
def global_setting(request):
    ...
    click_article_list = Article.objects.all().values('title', 'id').order_by('-click_count')[:4]
    # comment_article_list = comment(request)[:3]
    comment_article_list = Article.objects.all().values('title', 'id')[:4]
    recommend_article_list = Article.objects.all().filter(is_recommend = 1).order_by('-date_publish')[:5]
    # print type(recommend_article_list)
    # for a in recommend_article_list:
    #     print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", a, type(a), a.id
    #     print "+++++++++++++", hasattr(a, 'id'), hasattr(a, 'title')
...
def article(request):
    try:
        ...
        comment_count = Comment.objects.all().filter(art_id = id).count()
